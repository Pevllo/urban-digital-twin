...
model_defs = {
    ...
    "XGBoost": xgb.XGBRegressor(n_estimators=200, max_depth=8, learning_rate=0.1, min_child_weight=5, subsample=0.8, colsample_bytree=0.8, n_jobs=2, random_state=SEED, verbosity=0),
...
Xtr_t = torch.FloatTensor(Xtr_s)
ytr_t = torch.FloatTensor(y_train)
Xva_t = torch.FloatTensor(Xva_s)
yva_t = torch.FloatTensor(y_val)
Xte_t = torch.FloatTensor(Xte_s)
yte_t = torch.FloatTensor(y_test)
tr_ds = TensorDataset(Xtr_t, ytr_t)
va_ds = TensorDataset(Xva_t, yva_t)

class MLP(nn.Module):
    def __init__(self, d, hs=[256,128,64], dr=0.2):
        super().__init__()
        ls = []
        p = d
        for h in hs:
            ls += [nn.Linear(p, h), nn.BatchNorm1d(h), nn.ReLU(), nn.Dropout(dr)]
            p = h
        ls.append(nn.Linear(p, 1))
        self.net = nn.Sequential(*ls)
    def forward(self, x):
        return self.net(x).squeeze(-1)

def train_nn(m, tr_ds, va_ds, ep=60, bs=512, lr=1e-3, pat=10):
    tl = DataLoader(tr_ds, bs, shuffle=True)
    vl = DataLoader(va_ds, bs)
    opt = torch.optim.Adam(m.parameters(), lr=lr, weight_decay=1e-5)
    sch = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, patience=4, factor=0.5)
    crit = nn.MSELoss()
    best_val = float("inf")
    best_state = None
    cnt = 0
    for e in range(ep):
        ...
        with torch.no_grad():
            for xb, yb in vl:
                vl_loss += crit(m(xb), yb).item() * len(yb)
        vl_loss /= len(va_ds)
        sch.step(vl_loss)
        if vl_loss < best_val:
            best_val = vl_loss
...
t0 = time.time()
mlp = MLP(len(feature_cols), [256, 128, 64], 0.2)
mlp = train_nn(mlp, tr_ds, va_ds, ep=60, bs=512, lr=1e-3)
tt = time.time() - t0
mlp.eval()
with torch.no_grad():
    mlp_pv = mlp(Xva_t).numpy()
    mlp_pt = mlp(Xte_t).numpy()
r = calc_metrics("MLP", y_val, mlp_pv, tt, 0)
results.append(r)
print(" MAE=%.2f  R2=%.6f  (%.1fs)" % (r["MAE"], r["R2"], tt))
...
test_res.append(calc_metrics("MLP", y_test, mlp_pt))
...
best_name = None
for pref in ["LightGBM (Tuned)", "XGBoost (Tuned)", "HistGBM (Tuned)", "LightGBM", "XGBoost", "HistGradientBoosting"]:
